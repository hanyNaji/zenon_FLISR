...
###############------ CHANGE THIS ------###############
# file1 = r"D:\Zenon py\Line follower\FLISR\DAMMAM\STATISTIC DMM.xlsx"
output_file = r"D:\chnge order\FLISR\JUBAIL\ARAR\NOB_TRF_DB_FLISR.xlsx"
project_name = "SLD_TURIF_OFFICE#"
Administration = "NOB"
office_name = "TRF"
    ...
    if not pd.isna(feederID) and feederID != "-":
        ...
        if not matches.empty:
            ...
            if pd.notna(var_val) and str(var_val).strip() != "-":
                if Administration in ["DWD", "RUH", "QAS"]:
...
for idx, row in tqdm(df2.iterrows(), total=df2.shape[0], desc="Finishing up 1/2"):
    ...
    for i in range(2, 8):
        ...
        if con == "-":
            break
        if Administration in ["DWD", "RUH", "QAS"] and "ICCP" in con:
    ...
    if Administration in ["DWD", "RUH", "QAS"] and "ICCP" in row["Con1"]:
    ...
    for i in range(1, 8):
        ...
        if "#" in iso:
            if "_DCC#" in iso and "_OPN_CMD" in iso:
                ...
                iso_cmd = iso_cmd + iso + ","
            elif Administration in ["DWD", "RUH", "QAS"]:
                if "ICCP" in iso:
...
df2.drop(columns=['Con2', 'Con3', 'Con4', 'Con5', 'Con6', 'Con7', 'Con8', 'Con9', 'Con10', 'Con11', 'Con12', 'Con13', 'Con14', 'ISO1', 'ISO2', 'ISO3', 'ISO4', 'ISO5', 'ISO6', 'ISO7', 'ISO8', 'ISO9', 'ISO10', 'ISO11', 'ISO12', 'ISO13', 'ISO14', 'NOP_Variables',
                  'ID', 'NOP', 'feeder_id', 'Location Equipments IDs', 'first machine in feeder', 'last machines in feeder'], inplace=True)
...
## Clean VisualName columns
# df1['VisualName_clean'] = df1['VisualName'].astype(str).str.strip().str.lower()
# df2['VisualName_clean'] = df2['VisualName'].astype(str).str.strip().str.lower()
# # # Remove unwanted characters (O/S-, U/S-) from VisualName_clean
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r' ', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'#', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'PMT-', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'PMT_', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'PMT', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'[O/U/S]/S-', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'[O/U/S]/S_', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'[O/U/S]S-', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'[O/U/S]S_', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'[O/U/S]-', '', regex=True)
# # df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'[O/U/S]_', '', regex=True)
# # # REMOVE ANY CHARCTERS AT THE START OF THE STRING UNTIL THE FIRST NUMBER
# df1['VisualName_clean'] = df1['VisualName_clean'].str.replace(r'^[^\d]*', '', regex=True)
# df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'^[^\d]*', '', regex=True)
# # REMOVE ANY 0 AT THE START OF THE STRING
# df1['VisualName_clean'] = df1['VisualName_clean'].str.replace(r'^0000+', '', regex=True)
# df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'^0000+', '', regex=True)
# df1['VisualName_clean'] = df1['VisualName_clean'].str.replace(r'^000+', '', regex=True)
# df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'^000+', '', regex=True)
# df1['VisualName_clean'] = df1['VisualName_clean'].str.replace(r'^00+', '', regex=True)
# df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'^00+', '', regex=True)
# df1['VisualName_clean'] = df1['VisualName_clean'].str.replace(r'^0+', '', regex=True)
# df2['VisualName_clean'] = df2['VisualName_clean'].str.replace(r'^0+', '', regex=True)
# 
# # Prepare a lookup for the columns you want to add
# lookup = df1.set_index('VisualName_clean')[['FeederNo', 'Station', 'Office', 'Administration']]
# 
# # Fuzzy match each VisualName in df2 to df1
# def get_best_match(name, choices, threshold=100):
#     match = process.extractOne(name, choices, score_cutoff=threshold)
#     return match[0] if match else None
# 
# df2['best_match'] = df2['VisualName_clean'].apply(
#     lambda x: get_best_match(x, lookup.index)
# )
# 
# # Drop overlapping columns if they exist in df2
# for col in ['FeederNo', 'Station', 'Office', 'Administration']:
#     if col in df2.columns:
#         df2 = df2.drop(columns=[col])
# 
# df2 = df2.join(lookup, on='best_match')
# 
# df2.drop_duplicates(subset=["Picture", "ID"], inplace=True)
# 
# for idx, row in df2.iterrows():
...
#     FeederNo = row['FeederNo']
#
#     # Handle FeederNo cleaning - check if it's not NaN and not empty
#     if pd.notna(FeederNo) and str(FeederNo) != "-":
#         FeederNo = str(FeederNo)  # Convert to string first
#         # remove any whitespace without affecting the rest of the string
#         FeederNo = re.sub(r'\s+', '', FeederNo)
#         #  & if name must start with 'B'
#         if not FeederNo.startswith('B'):
#             FeederNo = 'B' + FeederNo
#         # remove anything except 'B', digits, and "()". only 1 'B'
#         FeederNo = re.sub(r'[^B\d()]', '', FeederNo)
#         FeederNo = re.sub(r'B+', 'B', FeederNo)
#         # if only 1 digit inside the parentheses, add '0' before it inside parentheses
#         if re.search(r'\(\d\)', FeederNo):
#             FeederNo = re.sub(r'\((\d)\)', r'(0\1)', FeederNo)
#         df2.at[idx, 'FeederNo'] = FeederNo
#    
#     if pd.isna(machine) or machine == "-":
#         continue
#     # Remove suffix if present
#     machine_base = machine.split(".")[0] if ".EF_ST" in machine else machine
#     machine_base = machine_base.split("_EF_")[0] if "_EF_ST" in machine_base else machine_base
# 
#     # Use regex to find _B... pattern _B125(05) followed by number only
#     # match = re.search(r'_B\d+\(\d+\)', machine_base)
#     match = re.search(r'_B\d+[^_]+', machine_base)
#     # # Use regex to find _B... pattern
#     # match = re.search(r'_B[^_]+', machine_base)
#     feeder = "-"
#     if match:
#         feeder = match.group(0)[1:]  # remove leading underscore
#         feeder = str(feeder)  # Convert to string first
#         # remove any whitespace without affecting the rest of the string
#         feeder = re.sub(r'\s+', '', feeder)
#         #  & if name must start with 'B'
#         if not feeder.startswith('B'):
#             feeder = 'B' + feeder
#         # remove anything except 'B', digits, and "()". only 1 'B'
#         feeder = re.sub(r'[^B\d()]', '', feeder)
#         feeder = re.sub(r'B+', 'B', feeder)
#         # if only 1 digit inside the parentheses, add '0' before it inside parentheses
#         if re.search(r'\(\d\)', feeder):
#             feeder = re.sub(r'\((\d)\)', r'(0\1)', feeder)
#     # Optionally, assign to a new column
#     df2.at[idx, 'FeederNo'] = feeder
# 
# # Preprocess FeederNo in df2
# df2['FeederNo_clean'] = df2['FeederNo'].fillna('000000').astype(str).str.split('(', n=1).str[0]
# 
# # Filter var_df for 'FDR' in LineRefName
# var_df_fdr = var_df[var_df['ID'].astype(str).str.contains('FDR', na=False)].copy()
# var_df_fdr['Variable'] = var_df_fdr['Variable'].astype(str)
# 
# # Build a lookup DataFrame for fast matching
# lookup_rows = []
# for _, row in var_df_fdr.iterrows():
#     for feeder in df2['FeederNo_clean'].unique():
#         if feeder in row['Variable']:
#             Variable = row['Variable']
#             Variable_OCT = "-"
#             Variable_CMD = "-"
#             if "#" in Variable and "_Y" not in Variable:
#                 Variable_OCT = Variable + "_OC_ST"
#                 Variable_CMD = Variable + "_OC_CMD"
#             lookup_rows.append({
#                 'Picture': row['ScreenName'],
#                 'FeederNo_clean': feeder,
#                 'Feeder OCT Variable': Variable_OCT,
#                 'Feeder CMD Variable': Variable_CMD
#             })
# 
# lookup_df = pd.DataFrame(lookup_rows)
# 
# # Merge to assign FeederVar
# df2 = df2.merge(lookup_df, on=['Picture', 'FeederNo_clean'], how='left')
# df2['Feeder OCT Variable'] = df2['Feeder OCT Variable'].fillna('-')
# df2['Feeder CMD Variable'] = df2['Feeder CMD Variable'].fillna('-')
# 
# # get feeder variables using Station if station in df2 contains same digits between parentheses as station in var_df
# for idx, row in df2.iterrows():
#     station = row['Station']
#     feeder = row['FeederNo']
#     if pd.isna(station) or station == "-":
#         continue
#     # Extract digits inside parentheses from station
#     station_digits = re.findall(r'\((\d+)\)', station)
#     feeder_digits = re.findall(r'\((\d+)\)', feeder)
#     if not station_digits or not feeder_digits:
#         continue
# 
#     # Use the actual digits without parentheses for matching
#     station_digit = station_digits[0]
#     feeder_digit = feeder_digits[0]
# 
#     # Find matching rows in var_df for station and feeder using exact digit match
#     station_pattern = f"\\({station_digit}\\)"  # Escape parentheses for regex
#     feeder_pattern = f"\\({feeder_digit}\\)"    # Escape parentheses for regex
#
#     matches = var_df[var_df['Station'].astype(str).str.contains(station_pattern, na=False) & 
#                      var_df['FeederNo'].astype(str).str.contains(feeder_pattern, na=False)]
# 
#     if not matches.empty:
#         df2.at[idx, 'Feeder OCT Variable'] = matches['Variable'].values[0] + "_OC_ST" if matches['Variable'].values[0] != "-" else "-"
#         df2.at[idx, 'Feeder CMD Variable'] = matches['Variable'].values[0] + "_OC_CMD" if matches['Variable'].values[0] != "-" else "-"
