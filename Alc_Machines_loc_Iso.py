...
def run(input_file_1, output_folder, use_scr_xml):
    ...
    for picture, picture_df in df.groupby("Picture"):
        ...
        for machine, connected_machines in machine_connections.items():
            ...
            if "FDR" not in machine and "INTEGRATION_PROJECT_NON_SMART_CB_SLD" not in machine:
    ...
    for idx, row in tqdm(output_df_1.iterrows(), total=output_df_1.shape[0], desc="Processing Isolations"):
        ...
        for i, smart_id in enumerate(smart_ids):
            ...
            if "FDR" in smart_var_id  or "INTEGRATION_PROJECT_NON_SMART_CB_SLD" in smart_var_id:
        ...
        for i in range(1, 15):
            ...
            if con != "-":
                ...
                # if "FDR" in parts[1] or "INTEGRATION_PROJECT_NON_SMART_CB_SLD" in parts[1]:
        ...
        for i in range(1, 15):
            con = row[f"Con{i}"]
            if i < len(valid_loc_vars)+1:
                output_df_1.at[idx, f"Con{i}"] = valid_loc_vars[i-1]
            else:
                output_df_1.at[idx, f"Con{i}"] = "-"
    ...
    output_df_1.to_excel(output_excel, index=False)
    print(f"Machines Database exported to {output_excel}")
